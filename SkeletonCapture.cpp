...
CBodyBasics::CBodyBasics() :
    ...
    m_pKinectSensor1(NULL),
	m_pKinectSensor2(NULL),
...
CBodyBasics::~CBodyBasics()
{
    ...
    if (m_pKinectSensor1)
    {
        m_pKinectSensor1->Close();
    }

    SafeRelease(m_pKinectSensor1);

	if (m_pKinectSensor2)
	{
		m_pKinectSensor2->Close();
	}

	SafeRelease(m_pKinectSensor2);
}
...
void CBodyBasics::writeToFile(IBodyFrame * bodyFrame)
{
}
...
void CBodyBasics::Update()
{
    ...
    if (SUCCEEDED(hr))
    {
        ...
        if (SUCCEEDED(hr))
        {
			//Able to get time and joint data so write that to a file for now
			//Can do some combined processing as an alternative option
			writeToFile(pBodyFrame);
...
HRESULT CBodyBasics::InitializeDefaultSensors()
{

/*
@TODO
Modify this to initialize two different Kinects -- need to modify the constructor as well 
*/
    HRESULT hr;

    hr = GetDefaultKinectSensor(&m_pKinectSensor1);
    if (FAILED(hr))
    {
        return hr;
    }
    ...
    if (m_pKinectSensor1)
    {
        ...
        hr = m_pKinectSensor1->Open();

        if (SUCCEEDED(hr))
        {
            hr = m_pKinectSensor1->get_CoordinateMapper(&m_pCoordinateMapper);
        }

        if (SUCCEEDED(hr))
        {
            hr = m_pKinectSensor1->get_BodyFrameSource(&pBodyFrameSource);
        }
...
void CBodyBasics::ProcessBody(INT64 nTime, int nBodyCount, IBody** ppBodies)
{
    if (m_hWnd)
    {
		...
		//Modify for multiple coordinate mappers
